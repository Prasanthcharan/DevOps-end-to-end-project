module "eks" {
  ...
  # Runner + jumpbox are both in private subnet, no public access needed
  ...
  addons = {
    ...
    aws-ebs-csi-driver = {
      most_recent = true
      # No service_account_role_arn here — using Pod Identity, not IRSA
      # Pod identity association is defined in module.ebs_csi_pod_identity below
    }
  }
...
resource "aws_security_group_rule" "jumpbox_to_eks" {
  ...
  description              = "Allow jumpbox to reach EKS API server"
}
...
resource "aws_security_group_rule" "runner_to_eks" {
  ...
  description              = "Allow GitHub Actions runner to reach EKS API server for kubectl deploys"
}
